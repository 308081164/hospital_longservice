#!/usr/bin/env python3
"""Apply phase-batch-p0.3.json patches to MySQL (utf8mb4)."""
...
SEED = ROOT / "backend/src/main/resources/billing-seeds/phase-batch-p0.3.json"
MARKER = "billing_seed_batch_p0_3_v1"
...
def mysql_file(sql_path: Path) -> None:
    subprocess.check_call(["docker", "cp", str(sql_path), "hospital-mysql:/tmp/p0_3.sql"])
    subprocess.check_call([
        "docker", "exec", "hospital-mysql", "sh", "-c",
        'mysql -uhospital -p"$MYSQL_PASSWORD" --default-character-set=utf8mb4 hospital < /tmp/p0_3.sql',
    ])
...
def main() -> int:
    ...
    print(f"Applying P0.3 ({len(lines)} statements)...")
    mysql_file(sql_path)
    sql_path.unlink()
