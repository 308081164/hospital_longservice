#!/usr/bin/env python3
"""Apply phase-batch-p0.5.1.json patches to MySQL (utf8mb4)."""
...
SEED = ROOT / "backend/src/main/resources/billing-seeds/phase-batch-p0.5.1.json"
MARKER = "billing_seed_batch_p0_5_1_v1"
...
def mysql_file(sql_path: Path) -> None:
    subprocess.check_call(["docker", "cp", str(sql_path), "hospital-mysql:/tmp/p0_5_1.sql"])
    subprocess.check_call([
        "docker", "exec", "hospital-mysql", "sh", "-c",
        'mysql -uhospital -p"$MYSQL_PASSWORD" --default-character-set=utf8mb4 hospital < /tmp/p0_5_1.sql',
    ])
...
def main() -> int:
    ...
    for patch in seed.get("ruleUpdates", []):
        ...
        if not row:
            ...
            continue
        parts = row.split("\t")
        keywords = parse_json_list(parts[0] if parts else "")
    ...
    print(f"Applying P0.5.1 ({len(lines)} statements)...")
    mysql_file(sql_path)
    sql_path.unlink()
