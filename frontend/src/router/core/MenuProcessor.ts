...
import { adaptBackendMenus, type BackendMenuNode } from './BackendMenuAdapter'
...
export class MenuProcessor {
  ...
  private async processBackendMenu(): Promise<AppRouteRecord[]> {
    ...
    const adapted = adaptBackendMenus(list as BackendMenuNode[])
    return this.filterEmptyMenus(adapted)
  }
  ...
  private filterEmptyMenus(menuList: AppRouteRecord[]): AppRouteRecord[] {
    return menuList
      ...
      .filter((item) => {
        ...
        // 如果有外链、iframe 或界面偏好入口，保留
        if (item.meta?.isIframe === true || item.meta?.link || item.meta?.openSettingPanel) {
          return true
        }
