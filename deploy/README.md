...
- MySQL（与 backend **同一连库目标**）：`billing_enabled=1` 为 **36**，P0.6 marker `billing_seed_batch_p0_6_v1` 存在。
- API：`GET /api/v1/customers` 统计 `billing_enabled=1` 为 **36**，且 **与 MySQL 计数一致**（UI 客户管理读此接口）。
...
| 文件 | 说明 |
|------|------|
| `deploy/sql/p0-6-billing-toggle.sql` | 幂等 SQL：36 院 `billing_enabled=1`，其余 `0`，写入 marker |
| `deploy/mysql-hospital-cli.sh` | 按 **运行中 backend 的 MYSQL_* ** 或 `.env` 执行 SQL/查询（避免 docker exec 打错库） |
| `deploy/apply-p0-6-billing-sql.sh` | 仅导入 P0.6 SQL 并检查 36 |
...
### 4.1 CI 显示 MySQL 36 院启用，浏览器仍约 10 院
...
浏览器：`http://39.102.213.51:8854` → 客户管理 → **特色账单已启用 36 / …**。
